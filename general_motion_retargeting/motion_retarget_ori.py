...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        if use_velocity_limit:
            VELOCITY_LIMITS = {k: 100*np.pi for k in self.robot_motor_names.keys()}
    ...
    def retarget(self, human_data, offset_to_ground=False):
        ...
        if self.use_ik_match_table1:
            ...
            vel1 = mink.solve_ik(
                self.configuration, self.tasks1, dt, self.solver, self.damping, safety_break=False,
                limits=self.ik_limits,
            )
            self.configuration.integrate_inplace(vel1, dt)
            next_error = self.error1()
            num_iter = 0
            while curr_error - next_error > 0.000001 and num_iter < self.max_iter:
        ...
        if self.use_ik_match_table2:
            ...
            dt = self.configuration.model.opt.timestep
            vel2 = mink.solve_ik(
                self.configuration, self.tasks2, dt, self.solver, self.damping, safety_break=False,
                limits=self.ik_limits
                )
